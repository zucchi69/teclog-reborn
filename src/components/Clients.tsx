...
export const Clients = () => {
  return <section id="clientes" className="py-24 bg-background">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {clients.map((client, index) => <div key={client.name} className="group bg-card rounded-2xl p-6 flex flex-col items-center justify-center border border-border hover:border-gold/50 hover:shadow-gold transition-all duration-300 hover:-translate-y-1 min-h-[140px]" style={{
          animationDelay: `${index * 50}ms`
        }}>
              <div className="w-full h-16 flex items-center justify-center mb-3 overflow-hidden">
                <img src={client.logo} alt={`Logo ${client.name}`} className={`max-w-full max-h-full object-contain grayscale group-hover:grayscale-0 transition-all duration-300 ${client.scale}`} />
              </div>
        ...
        <div className="mt-16 bg-gradient-hero rounded-3xl p-10 md:p-14 text-center">
          ...
          <a href="#contato" className="inline-flex items-center gap-2 font-semibold px-8 py-4 rounded-xl transition-colors shadow-gold bg-[#b3dfe6] text-secondary-foreground">
